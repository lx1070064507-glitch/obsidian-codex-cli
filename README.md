...
- 本机文件读取使用全盘只读权限档案。
- 可以把多个本机目录加入同一会话的工作区。
- 工作区内的白名单文件或目录默认允许创建、修改、重命名和删除，不显示审批弹窗。
- 未命中白名单的写入和网络访问需要在 Obsidian 中逐次审批。
...
1. 双击桌面的“Open Codex Vault”快捷方式，或运行仓库根目录的 `打开 Codex Vault.cmd`。
2. 在文件夹选择器中选择本机项目目录。
3. 启动器会创建或更新该目录的 `.obsidian` 插件运行文件并启用 Codex CLI，然后使用 Obsidian 打开该 Vault。

目标已经安装当前版本时不会重复写入。启动器只更新 `main.js`、`manifest.json`、`styles.css` 和插件启用列表，不会覆盖目标 Vault 的 `data.json`、其他插件、插件设置或项目文件。

也可以从命令行使用：

```powershell
.\tools\Open-CodexVault.ps1 -VaultPath "D:\Project"
```

如果提示来源插件文件不存在，先在本仓库的 `plugin/` 目录运行 `npm run build`。

### 管理工作区与写入白名单

1. 打开 Obsidian 设置中的 Codex CLI 插件设置页。
2. 点击“管理工作区”，添加需要在同一会话中使用的本机项目目录。
3. 点击“管理白名单”，添加工作区内默认可写的现有文件或目录。
4. 保存后，下一次发送消息会使用新的工作区和权限配置。

工作区和白名单默认均为空。白名单必须位于某个工作区内；加入白名单后，创建、修改、重命名和删除均不会显示审批弹窗。如需创建尚不存在的文件，应将其现有父目录加入白名单。

工作区管理只控制 Codex 会话可使用的额外目录，不负责把目录初始化为独立 Vault。需要安装并打开独立 Vault 时使用外部启动器。
...
- 工作区用于把多个目录组织进同一会话，但不会自动授予写权限。
- 白名单真实路径默认可写，其中创建、修改、重命名和删除不会触发审批。
- 工作区内非白名单写入和工作区外写入都会触发 Obsidian 审批弹窗。
- 网络访问继续逐次审批。
...
│   └── styles.css          # 插件样式
├── tools/                  # 独立 Vault 启动器、快捷方式安装器和测试
├── 打开 Codex Vault.cmd   # Windows 双击入口
├── docs/                   # 设计方案、实施计划和验收记录
...
- 第一版只提供全局工作区和白名单列表，不支持按会话切换配置、路径通配符或路径拖拽。
