# Obsidian Codex CLI 集成设计

日期：2026-08-15
目标目录：`D:\My_DateBase\Obsidian_CodexCli`
状态：已实现；自动化测试、真实 Git 和本机 Codex 握手通过；Obsidian 桌面手工验收待安装后执行

## 1. 目标

构建一个仅支持 Windows 桌面版 Obsidian 的插件，将 Obsidian Vault 与本机安装的 Codex CLI 连接起来。插件提供原生聊天侧边栏，把完整临时对话和经过确认的对话成果分别保存到两个目录。临时对话只留在本机；成果可以在 Obsidian 中随时查看，并由用户选择性提交到 Git。

本设计把 Markdown 文件作为可长期保存的项目记忆，把 Codex 会话作为执行上下文，把 Git 作为审计和同步层。设计不依赖本地 CLI 对话出现在 ChatGPT 聊天历史中。

## 2. 已确认范围

第一版将实现：

- 仅支持 Windows 桌面版 Obsidian。
- 要求安装 `codex.cmd`、Codex CLI `0.147.0` 和 Git。
- 通过本地子进程使用实验性的 Codex `app-server` 协议。
- 提供原生 Obsidian 右侧聊天栏。
- 每个会话都与当前打开的 Markdown 笔记关联。
- 将完整临时对话保存到 `Codex Chats/`，并始终排除在 Git 之外。
- 只有在用户明确确认后，才把选定的 Codex 回复保存为 `Codex Results/` 下的独立成果笔记。
- 在当前笔记中只添加成果笔记的 Obsidian 链接，不复制完整成果正文。
- 以 Vault 作为 Codex 工作区，但自定义权限档案不授予任何默认写入能力。
- 使用 Codex CLI 原生 `on-request` 审批策略和自定义 `obsidian-vault` 权限档案：本机任意路径的文件读取默认执行；Vault 内外的网络、写入、修改和删除显示审批请求。
- 第一版只提供“允许一次”和“拒绝”，不提供永久授权。
- 只有在用户确认后才预览并创建 Git commit。
- Git 提交界面只允许选择 `Codex Results/` 下的成果文件，不提交临时会话或个人笔记。

第一版不支持移动端 Obsidian、macOS、Linux、多 Vault、语义检索、永久权限规则、普通 ChatGPT 对话自动同步，也不会静默降级为不受控的 CLI 执行方式。

## 3. 仓库结构

```text
D:\My_DateBase\Obsidian_CodexCli\
|-- .git\
|-- .gitignore
|-- .obsidian\
|   `-- plugins\
|       `-- obsidian-codex-cli\
|           |-- manifest.json
|           |-- main.js
|           `-- styles.css
|-- Codex Chats\
|-- Codex Results\
|-- plugin\
|   |-- src\
|   |   |-- main.ts
|   |   |-- chat-view.ts
|   |   |-- codex-client.ts
|   |   |-- transcript-store.ts
|   |   |-- result-store.ts
|   |   |-- context-service.ts
|   |   |-- git-service.ts
|   |   |-- health-check.ts
|   |   `-- settings.ts
|   |-- tests\
|   |-- package.json
|   |-- tsconfig.json
|   `-- esbuild.config.mjs
|-- docs\
|   `-- superpowers\
|       `-- specs\
`-- AGENTS.md
```

插件源代码保存在 `plugin/`。构建过程把 Obsidian 运行所需的三个文件写入 `.obsidian/plugins/obsidian-codex-cli/`。

仓库采用白名单式 `.gitignore`：只跟踪 `plugin/`、必要构建配置、`AGENTS.md`、`docs/` 和用户明确选择提交的 `Codex Results/` 成果文件。`Codex Chats/`、个人笔记、其他 Vault 内容、`.obsidian/` 工作区状态、插件构建产物、依赖目录和本地插件设置均只保存在本机。

## 4. 组件设计

### 4.1 聊天视图

右侧聊天栏负责展示和用户交互，包括健康状态、消息、流式输出、停止状态、审批请求、重新连接、“保存为成果”、成果列表、提交预览和最终提交结果。界面使用 Obsidian 主题变量和桌面原生控件，不引入独立设计系统。

同一会话同时只允许执行一个 Codex 回合。关闭聊天栏不会删除会话记录或会话元数据。

### 4.2 Codex 客户端

Codex 客户端以本地子进程启动 `codex.cmd app-server`，并通过结构化的标准输入输出协议通信。它负责进程生命周期、协议消息、线程创建和恢复、流式事件、取消操作、审批响应及协议错误。

所有实验性协议细节都封装在这一适配层内。如果已安装的 Codex 版本不完全等于 `0.147.0`，或者协议握手失败，插件将拒绝启动会话。插件不会降级到 `danger-full-access`、免审批参数或解析终端文本的非结构化方式。

### 4.3 会话记录存储

每个会话保存为 `Codex Chats/YYYY-MM-DD-<净化后的标题>.md`。笔记开头使用 YAML 元数据记录 Codex 会话 ID、创建时间、更新时间、关联笔记路径、Codex 返回的模型信息和会话状态。

用户消息在发送前落盘。Codex 回复在流式生成期间持续更新，并在回合结束后写入最终内容。记录中包含用户可见的审批结果和简洁的执行摘要。原始协议帧、密钥、环境变量和凭证绝不写入会话笔记。

### 4.4 成果存储

用户点击“保存为成果”后，插件先显示可编辑预览。确认后在 `Codex Results/` 创建独立 Markdown，记录成果标题、创建时间、来源会话、关联笔记和成果正文。插件随后只在当前笔记中加入指向该成果的 Obsidian 链接。

成果笔记是唯一可以通过插件选择性提交的数据文件。成果创建后持续保留在 Vault 中，并可从插件成果列表或 Obsidian 文件浏览器随时打开。

### 4.5 上下文服务

创建新会话时必须存在当前打开的 Markdown 笔记。第一轮向 Codex 发送关联笔记的路径和当前内容。后续回合只发送新的用户消息；如果关联笔记发生变化，则只补充一次更新后的笔记内容。插件不会自动扫描整个 Vault。

Codex `app-server` 线程保留对话上下文，因此插件不会在每一轮重复发送完整会话记录。

### 4.6 Git 服务

Git 服务只读取 `Codex Results/` 下的成果文件。提交界面列出尚未提交或已有修改的成果，允许用户逐个选择，并显示准确的文件列表、差异和可编辑的提交说明。用户确认后，服务只暂存被选中的成果文件。

如果已跟踪的候选成果在本次编辑前已经存在未提交修改，插件将阻止自动提交，避免把旧修改混入当前 commit。`Codex Chats/`、当前笔记和其他已修改或未跟踪文件不会进入暂存命令。

### 4.7 健康检查与设置

启动检查包括：

- 插件运行在 Windows 桌面版 Obsidian 中。
- 配置的 `codex.cmd` 存在，并返回 `codex-cli 0.147.0`。
- Codex 登录状态可用。
- Git 可用。
- Vault 位于预期的 Git 仓库中。
- Codex `app-server` 协议握手成功。

设置页允许用户明确指定 `codex.cmd` 和 Git 的路径，默认使用命令发现。设置只保存在本机，不提交到 Git。

## 5. 用户流程

1. 用户打开一个 Markdown 笔记并激活 Codex 侧边栏。
2. 用户新建会话，插件在 `Codex Chats/` 创建只保存在本机的会话笔记。
3. 用户发送消息，消息在交给 Codex 前先写入会话笔记。
4. Codex 客户端以 Vault 为工作根目录，创建或恢复 `app-server` 线程。
5. 流式回复显示在侧边栏中，并同步保存到会话笔记。
6. 审批请求会暂停当前回合，并显示具体操作、目标和原因。
7. 用户选择“允许一次”或“拒绝”，插件把决定返回给 Codex，并将简要结果写入会话记录；拒绝时同时中止当前回合，避免重复申请。
8. 完成的 Codex 回复提供“保存为成果”操作。
9. 保存成果时先打开可编辑预览，确认后在 `Codex Results/` 创建独立成果笔记，并在当前笔记中添加该成果的链接。
10. 成果列表可以随时打开、查看和编辑已有成果。
11. 点击提交后，插件只显示 `Codex Results/` 下符合条件的成果文件；用户逐个选择并检查差异和提交说明后确认 commit。

## 6. 权限模型

Codex 以 Vault 为工作区根目录，使用 `approvalPolicy: "on-request"`、`approvalsReviewer: "user"` 和自定义 `obsidian-vault` 权限档案。档案只对 `:root` 授予只读访问，不授予任何默认写入能力，并关闭命令网络。第一版不启用 Codex 原生网页搜索。Vault 内外的网络、写入、修改和删除必须触发 `app-server` 审批请求。

Windows 桌面实测证明，`untrusted` 会在 PowerShell 读取 Vault 外文件时按命令信誉触发审批，即使自定义档案已经允许 `:root` 只读；同时把 `:workspace_roots` 设为 `write` 会让 Vault 内写入无需确认。CLI 帮助中出现的 `sandbox_permissions` 示例在 `0.147.0` 严格配置检查下属于未知字段。插件最终采用官方支持的全盘只读档案配合 `on-request`，且仍不得解析命令字符串或自动批准请求。线程选择权限档案后不得同时传入旧 `sandbox` 字段。

插件不解析命令字符串、不维护只读白名单，也不自动批准收到的请求。审批只对当前请求生效，插件不保存永久允许规则。Git commit 只能从插件界面主动发起，且只能包含用户选中的成果文件；发送给 Codex 的项目规则明确禁止 Codex 自行提交、推送、删除文件或修改仓库配置。

插件绝不启用 `--dangerously-bypass-approvals-and-sandbox`、`danger-full-access`、`approvalPolicy: "never"` 或等效配置。

## 7. 失败处理

- Codex 缺失或版本不兼容：禁用发送，并显示检测到的路径和版本。
- Git 或仓库缺失：允许聊天、保存临时会话和保存成果，但禁用成果提交。
- Codex 未登录：禁用发送，显示相应的 Codex 登录命令，但不尝试读取凭证。
- `app-server` 退出：保留会话笔记，并使用已保存的会话 ID 提供“重新连接并恢复”。
- 收到未知协议消息：停止当前请求，保存经过净化的错误摘要，并要求更新到兼容的插件版本。
- Obsidian 退出或插件卸载：先请求子进程正常退出；若其未在短时间内结束，再终止子进程。已保存笔记保持不变。
- 提交候选成果已有旧修改：阻止自动提交，并明确指出受影响文件。
- Git commit 失败：除了 Git 已完成的暂存外，不改变工作区；显示错误，并且只有在用户确认后才提供安全的取消暂存操作。

## 8. 测试设计

### 8.1 单元测试

- 临时会话文件命名、YAML 元数据、内容追加、流式回复结束处理和敏感信息净化。
- 成果文件命名、来源元数据、正文保存和当前笔记链接生成。
- Vault 路径包含关系和关联笔记变更检测。
- 成果候选文件筛选、逐项选择和旧修改拦截。
- 提交说明生成和精确的暂存参数构造。
- 健康检查中的版本解析和可执行文件发现。

### 8.2 协议测试

使用模拟 `app-server` 进程覆盖初始化、线程创建、线程恢复、流式回复、取消、允许一次、拒绝、进程退出、格式错误消息和未知消息。

### 8.3 Git 集成测试

在临时仓库中验证：`Codex Chats/` 和个人笔记始终被忽略；只暂存并提交用户选中的 `Codex Results/` 成果；无关修改保持不变；候选成果存在旧修改时阻止提交。

### 8.4 构建验证

项目必须通过类型检查、自动化测试和生产构建。Vault 插件目录中必须生成 `manifest.json`、`main.js` 和 `styles.css`。

## 9. 验收标准

1. Obsidian 能加载插件并打开 Codex 侧边栏。
2. 健康检查能准确识别 Codex CLI `0.147.0`、登录状态和 Git。
3. 从当前 Markdown 笔记开始会话时，会在 `Codex Chats/` 下创建关联的临时会话笔记，该文件不会出现在 Git 候选列表中。
4. 关闭并重新打开 Obsidian 后，已发送消息和已生成回复不会丢失。
5. 重新连接后可以恢复已保存的 Codex 会话。
6. 命令读取 Vault 内外本机文件时不弹窗；Vault 内外写入和网络访问必须显示 Obsidian 审批窗口。
7. 只有在用户预览并确认后，“保存为成果”才在 `Codex Results/` 创建独立成果，并在当前笔记中加入链接。
8. 已保存成果可以从插件成果列表和 Obsidian 文件浏览器随时查看。
9. 提交预览只列出 `Codex Results/` 下可提交的成果，并允许逐项选择。
10. 确认后的 commit 不包含临时会话、个人笔记或仓库中的其他修改。
11. 候选成果存在本次编辑前的旧修改时，自动提交必须被阻止。

## 10. 已知限制

Codex `app-server` 目前仍是实验接口。第一版以 `0.147.0` 作为已验证的协议基线。支持更新的 Codex 版本前，必须重新运行协议测试和端到端测试，再扩大插件接受的版本范围。
