...
it("真实 Git 仓库只提交用户选择的成果", async () => {
  ...
  try {
    ...
    await writeFile(join(root, "plugin", "Unrelated.md"), "original", "utf8");
    ...
    await run(runner, root, ["add", "--", ".gitignore", "plugin/Unrelated.md"]);
    await run(runner, root, ["commit", "-m", "init"]);

    const service = new GitService(root, "git", runner);
    await service.captureBaseline();
    await writeFile(join(root, "plugin", "Unrelated.md"), "staged unrelated change", "utf8");
    await run(runner, root, ["add", "--", "plugin/Unrelated.md"]);
    ...
    const staged = await run(runner, root, ["diff", "--cached", "--name-only"]);
    expect(staged.stdout.trim()).toBe("plugin/Unrelated.md");
